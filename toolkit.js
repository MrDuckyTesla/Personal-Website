...
class ToolKit {  // USED TO BE THE MEDIA CLASS, CHANGED DUE TO IT BEING EXPANDED TO MORE OF A GAME ENGINE
  constructor() {
  ...
  preCompile(img, layerList) {  // PreCompile to avoid lag
    ...
    let len1 = img.pixels.length, len2 = layerList.length;  // Dont recalculate every iteration
    for (let i = 0; i < len1; i += 4) {  // Iterate through pixels
      if (img.pixels[i + 3] != 0) {  // Check if pixel is transparent
        for (let j = 0; j < len2; j ++) {  // Iterate through color layers
  ...
  changeColor(img, colorList, tintList) {
    ...
    let len = colorList.length;  // Dont recalculate every iteration
    for (let i = 0; i < len; i += 3) {  // Apply formula:  NR = (3 * (G + r) - (g + b)) / 4
  ...
  changeSingleColor(img, ogColorRGB, newColorRGB) {
    ...
    for (let i = 0; i < len; i += 4) {  // Iterate through pixels
      if (img.pixels[i+3] != 0) {  // Check if pixel is transparent
  ...
  circleImage(x, y, res, thicc, sizeDisplay, clr1=[255, 111, 111, 255], grad=false, clr2=[111, 111, 255, 255]) {
    ...
    for (let i = 0; i < len; i += 4) {  // Iterate through pixel list
      ...
      if (dis <= thicc) {
        ratio = dis/thicc;  // Calculate ratio of radius to color
        // Change colors
        img.pixels[i] = grad? clr1[0] : lerp(clr1[0], clr2[0], ratio);
        img.pixels[i+1] = grad? clr1[1] : lerp(clr1[1], clr2[1], ratio);
        img.pixels[i+2] = grad? clr1[2] : lerp(clr1[2], clr2[2], ratio);
        img.pixels[i+3] = grad? clr1[3] : lerp(clr1[3], clr2[3], ratio);
      }
    }
    img.updatePixels();  // Update image
    return img;
  ...
  outline(img, clr1=[0, 0, 0, 0], clr2=[255, 255, 255, 255]) {
    ...
    for (let i = 0; i < len; i += 4) {
      if (img.pixels[i+3] != 0 && !this.compareColorList(img.pixels[i], img.pixels[i+1], img.pixels[i+2], img.pixels[i+3], clr2)) {  // Dont check if transparent pixel
        ...
        else if (this.compareColorList(img.pixels[i+img.width*4], img.pixels[i+img.width*4+1], img.pixels[i+img.width*4+2], img.pixels[i+img.width*4+3], clr1)) {
          img.pixels[i+img.width*4] = clr2[0];
          img.pixels[i+img.width*4+1] = clr2[1];
          img.pixels[i+img.width*4+2] = clr2[2];
          img.pixels[i+img.width*4+3] = clr2[3]; 
  ...
  rectRectCollideCoords(px, py, x1, y1, w1, h1, x2, y2, w2, h2) {
    ...
    let len = coordCollide.length;  // Dont recalculate every iteration
    for (let i = 0; i < len; i++) {
  ...
  compareListList(list1, list2) {
    ...
    let len = list1.length;  // Dont recalculate every iteration
    for (let i = 0; i < len; i ++) {
